...
def entity_table():
    try:
        if request.method == 'POST':
            ...
            formlist = (formData['scrip_name'],formData.get('scripcode'),None,formData.get('benchmark'),formData['category'],formData.get('subCategory'),formData.get('nickname'),datetime.now()
            )
