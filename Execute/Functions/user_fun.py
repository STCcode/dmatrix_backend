...
def insertMFNavData():
    try:
        if request.method == 'POST':
            ...
            data_tuple = (isin, nav_value, nav_date)
            print("DEBUG NAV INSERT:", data_tuple)

            success = queries.insert_MF_NavData(data_tuple)
...
def getAllMFEquitytotalValue():
    if request.method == 'GET':
        try:
            data = queries.getAllMFEquitytotalValue()
...
def getAIFEquityDetailsById():
    try:
        ...
        # Query the database
        data = queries.getAIFEquityDetailsById(entity_id)
...
def  getAIFFixedIncomeDetailsById ():
    try:
        entity_id = None
        ...
        data = queries.getAIFFixedIncomeDetailsById (entity_id)
...
def getAifDetailsFixedIncomeUnderlyingTable():
    try:
        entity_id = None
        ...
        # Query the database
        data = queries.getAifDetailsFixedIncomeUnderlyingTable(entity_id)
...
#         # Query the database
#         data = queries.getETFDetailsById(entity_id)
...
def getAllActionTableOfETFEquity():
     if request.method == 'GET':
        try:
            data=queries.getAllActionTableOfETFEquity()
            if type(data).__name__  != "list":
                if data.json:
                    result=data
                    status=500
...
def  getETFDetailsEquityById ():
    try:
        ...
        data = queries.getETFDetailsEquityById (entity_id)
...
def getETFEquityDetailUnderlyingTable():
    try:
        ...
        # Query the database
        data = queries.getETFEquityDetailUnderlyingTable(entity_id)
...
def getCountOfAllFixedIncome():
     if request.method == 'GET':
        try:
            data=queries.getCountOfAllFixedIncome()
            if type(data).__name__  != "list":
                if data.json:
                    result=data
                    status=500
...
def insertPmsAmcAction():
    try:
        if request.method == 'POST':
            ...
            formlist = (formData['entityid'],formData['scrip_name'],formData['scrip_code'],formData['order_type'],formData['quantity'],formData['trade_price'],formData['net_amount'],
                datetime.now()
            )
...
def getAllEquityUnderlyingCount():
     if request.method == 'GET':
        try:
            data=queries.getAllEquityUnderlyingCount()
...
def GetallMfSectorUnderlyingCount():
     if request.method == 'GET':
        try:
            data=queries.GetallMfSectorUnderlyingCount()
...
def getallMFDetailsEquityMCAPCount():
    try:
        entity_id = None
        ...
        # Query the database
        data = queries.getallMFDetailsEquityMCAPCount(entity_id)
...
def GetallAIFEquityUnderlyingCount():
     if request.method == 'GET':
        try:
            data=queries.GetallAifEquityUnderlyingCount()
...
def GetallAIFSectorUnderlyingCount():
     if request.method == 'GET':
        try:
            data=queries.GetallAIFSectorUnderlyingCount()
...
def GetallAIFFixedIncomeSectorCount():
     if request.method == 'GET':
        try:
            data=queries.GetallAIFFixedIncomeSectorCount()
...
def getAIFDetailsFixedIncomeSectorCount():
    try:
        ...
        # Query the database
        data = queries.getAIFDetailsFixedIncomeSectorCount(entity_id)
