...
def getAIFDetailsFixedIncomeSectorCount():
     try:
        sql="WITH latest_nav AS ( SELECT DISTINCT ON (isin)     isin,     post_tax_nav AS nav FROM tbl_aif_nav ORDER BY isin, nav_date DESC),entity_values AS ( SELECT     a.entityid,     SUM(a.num_units * COALESCE(nav.nav, 1)) AS entity_total_value FROM tbl_aif a LEFT JOIN latest_nav nav ON a.isin = nav.isin JOIN tbl_entity e ON a.entityid = e.entityid WHERE e.category = 'Fixed_Income'   AND e.subcategory = 'Alternative Investment Funds' GROUP BY a.entityid),total_value AS ( SELECT SUM(entity_total_value) AS total_all_entities FROM entity_values),entity_mf_percent AS ( SELECT     ev.entityid,     ev.entity_total_value,     (ev.entity_total_value / tv.total_all_entities) AS mf_weight_fraction,     ROUND((ev.entity_total_value / tv.total_all_entities) * 100, 2) AS mf_weight_percent FROM entity_values ev CROSS JOIN total_value tv),entity_sectors AS ( SELECT     u.entityid,     u.sector,     COUNT(*) AS sector_count,     SUM(u.weightage::numeric) AS sector_weight FROM tbl_underlying u JOIN tbl_entity e ON u.entityid = e.entityid WHERE u.sector IS NOT NULL   AND e.category = 'Fixed_Income'   AND e.subcategory = 'Alternative Investment Funds' GROUP BY u.entityid, u.sector),entity_totals AS ( SELECT entityid, SUM(sector_weight) AS total_sector_weight FROM entity_sectors GROUP BY entityid),entity_sector_breakdown AS ( SELECT     es.entityid,     es.sector,     es.sector_count,     es.sector_weight,     et.total_sector_weight,     emp.mf_weight_percent,     (es.sector_weight / et.total_sector_weight) * emp.mf_weight_fraction AS sector_fraction FROM entity_sectors es JOIN entity_totals et ON es.entityid = et.entityid JOIN entity_mf_percent emp ON es.entityid = emp.entityid),sector_totals AS ( SELECT     sector,     SUM(sector_count) AS total_sector_count,     ROUND(SUM(sector_fraction) * 100, 2) AS overall_sector_percent FROM entity_sector_breakdown GROUP BY sector),grand_total AS ( SELECT SUM(total_sector_count) AS overall_sector_count FROM sector_totals)SELECT st.sector, st.total_sector_count, gt.overall_sector_count, st.overall_sector_percent FROM sector_totals st CROSS JOIN grand_total gt ORDER BY st.overall_sector_percent DESC;"       
        data=''
        msgs=executeSql.ExecuteAllNew(sql,data)
        return msgs
     except Exception as e:
