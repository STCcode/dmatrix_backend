...
def getallMFDetailsEquitySectorCount(entity_id):
    try:
        ...
        sql="WITH entity_tag_weight AS (SELECT u.tag,SUM(u.weightage::numeric) AS tag_weight FROM tbl_underlying u WHERE u.entityid = %s AND u.tag IS NOT NULL GROUP BY u.tag),global_total AS (SELECT SUM(weightage::numeric) AS total_weight FROM tbl_underlying WHERE tag IS NOT NULL)SELECT etw.tag,etw.tag_weight,gt.total_weight,(etw.tag_weight * 100.0 / gt.total_weight)::numeric(7,2) AS tag_percent FROM entity_tag_weight etw CROSS JOIN global_total gt;"
