...
def insertClientAction(data):
    try:
        sql = " INSERT INTO tbl_pms_client_action (entityid, order_type, trade_price, cheque, created_at, trade_date) VALUES (%s, %s, %s, %s, %s, %s)"
        msg = executeSql.ExecuteOne(sql, data)
        return msg
    except Exception as e:
        print("Error in save_user query==========================", e)
...
def insert_entity_return_id(entity_data):
    """
    entity_data: (scripname, scripcode, benchmark, category, subcategory, nickname, isin, created_at)
    Returns: entityid generated by DB
    """
    sql = """
        ...
        RETURNING entityid
    """
    result = executeSql.ExecuteOne(sql, entity_data)
    if result:
...
def insert_pdf_file(entityid, pdf_name, pdf_bytes, uploaded_at):
    sql = """
        ...
        VALUES (%s, %s, %s, %s)
    """
    executeSql.ExecuteOne(sql, (entityid, pdf_name, pdf_bytes, uploaded_at))
...
def auto_action_table(action_data):
    ...
    """
    executeSql.ExecuteOne(sql, action_data)


def get_entity_by_category_subcategory(category, subcategory):
    sql = "SELECT entityid FROM tbl_entity WHERE category=%s AND subcategory=%s ORDER BY id DESC LIMIT 1"
    return executeSql.ExecuteOne(sql, (category, subcategory))
# =================== AIF ===================
