...
def getallMFDetailsEquityMCAPCount(entity_id):
    try:
        ...
        sql="WITH all_tags AS (SELECT DISTINCT tag FROM tbl_underlying WHERE tag IS NOT NULL),entity_counts AS ( SELECT u.tag, COUNT(*) AS tag_count FROM tbl_underlying u JOIN tbl_entity e ON u.entityid = e.entityid WHERE e.category = 'Equity' AND e.subcategory = 'Mutual Fund' AND u.entityid = %s AND u.tag IS NOT NULL GROUP BY u.tag), total AS (SELECT COUNT(*) AS total_tag_count FROM tbl_underlying u JOIN tbl_entity e ON u.entityid = e.entityid WHERE e.category = 'Equity' AND e.subcategory = 'Mutual Fund' AND u.entityid = 'ENT-0333' AND u.tag IS NOT NULL) SELECT t.tag, COALESCE(ec.tag_count, 0) AS tag_count,total.total_tag_count,COALESCE((ec.tag_count * 100.0 / total.total_tag_count)::numeric(5,2), 0.00) AS tag_percent FROM all_tags t LEFT JOIN entity_counts ec ON t.tag = ec.tag CROSS JOIN total ORDER BY t.tag;"
