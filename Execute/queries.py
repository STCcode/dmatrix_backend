...
def entity_table(data):
    try:
        sql = "INSERT INTO tbl_entity (scripname, scripcode, entityID, benchmark_name, category,subcategory, nickname, created_at, isin,aif_category, aif_class) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
        msg = executeSql.ExecuteReturnId(sql, data)
        return msg
    except Exception as e:
        print("Error in save_user query==========================", e)
...
def GetallAifEquityUnderlyingCount():
    try:
        sql="WITH latest_nav AS (SELECT DISTINCT ON (isin)	isin,	post_tax_nav AS navFROM tbl_aif_navORDER BY isin, nav_date DESC),entity_values AS (SELECT	a.entityid,	SUM(a.num_units * COALESCE(nav.nav, 1)) AS entity_total_valueFROM tbl_aif aLEFT JOIN latest_nav nav ON a.isin = nav.isinJOIN tbl_entity e ON a.entityid = e.entityidWHERE e.category = 'Equity'  AND e.subcategory = 'Alternative Investment Funds'GROUP BY a.entityid),total_value AS (SELECT SUM(entity_total_value) AS total_all_entitiesFROM entity_values),entity_mf_percent AS (SELECT	ev.entityid,	ev.entity_total_value,	(ev.entity_total_value / tv.total_all_entities) AS mf_weight_fraction,	ROUND((ev.entity_total_value / tv.total_all_entities) * 100, 2) AS mf_weight_percentFROM entity_values evCROSS JOIN total_value tv),entity_weights AS (SELECT	u.entityid,	u.tag,	COUNT(*) AS tag_count,	SUM(u.weightage::numeric) AS tag_weightFROM tbl_underlying uJOIN tbl_entity e ON u.entityid = e.entityidWHERE u.tag IS NOT NULL  AND e.category = 'Equity'  AND e.subcategory = 'Alternative Investment Funds'GROUP BY u.entityid, u.tag),entity_totals AS (SELECT entityid, SUM(tag_weight) AS total_tag_weightFROM entity_weightsGROUP BY entityid),entity_tag_breakdown AS (SELECT	ew.entityid,	ew.tag,	ew.tag_count,	ew.tag_weight,	et.total_tag_weight,	emp.mf_weight_percent,	(ew.tag_weight / et.total_tag_weight) * emp.mf_weight_fraction AS tag_fractionFROM entity_weights ewJOIN entity_totals et ON ew.entityid = et.entityidJOIN entity_mf_percent emp ON ew.entityid = emp.entityid),tag_totals AS (SELECT	tag,	SUM(tag_count) AS total_tag_count,	ROUND(SUM(tag_fraction) * 100, 2) AS overall_tag_percentFROM entity_tag_breakdownGROUP BY tag),grand_total AS (SELECT SUM(total_tag_count) AS overall_tag_countFROM tag_totals)SELECTtt.tag,tt.total_tag_count,gt.overall_tag_count,tt.overall_tag_percentFROM tag_totals ttCROSS JOIN grand_total gtORDER BY tt.overall_tag_percent DESC;"      
        data=''       
        msgs=executeSql.ExecuteAllNew(sql,data)        
        return msgs    
    except Exception as e:
...
def GetallAIFSectorUnderlyingCount():
     try:
        sql="WITH latest_nav AS (SELECT DISTINCT ON (isin)    isin,    post_tax_nav AS navFROM tbl_aif_navORDER BY isin, nav_date DESC),entity_values AS (SELECT    a.entityid,    SUM(a.num_units * COALESCE(nav.nav, 1)) AS entity_total_valueFROM tbl_aif aLEFT JOIN latest_nav nav ON a.isin = nav.isinJOIN tbl_entity e ON a.entityid = e.entityidWHERE e.category = 'Equity'  AND e.subcategory = 'Alternative Investment Funds'GROUP BY a.entityid),total_value AS (SELECT SUM(entity_total_value) AS total_all_entitiesFROM entity_values),entity_mf_percent AS (SELECT    ev.entityid,    ev.entity_total_value,    (ev.entity_total_value / tv.total_all_entities) AS mf_weight_fraction,    ROUND((ev.entity_total_value / tv.total_all_entities) * 100, 2) AS mf_weight_percentFROM entity_values evCROSS JOIN total_value tv),entity_weights AS (SELECT    u.entityid,    u.sector,    COUNT(*) AS tag_count,    SUM(u.weightage::numeric) AS tag_weightFROM tbl_underlying uJOIN tbl_entity e ON u.entityid = e.entityidWHERE u.sector IS NOT NULL  AND e.category = 'Equity'  AND e.subcategory = 'Alternative Investment Funds'GROUP BY u.entityid, u.sector),entity_totals AS (SELECT entityid, SUM(tag_weight) AS total_tag_weightFROM entity_weightsGROUP BY entityid),entity_tag_breakdown AS (SELECT    ew.entityid,    ew.sector,    ew.tag_count,    ew.tag_weight,    et.total_tag_weight,    emp.mf_weight_percent,    (ew.tag_weight / et.total_tag_weight) * emp.mf_weight_fraction AS tag_fractionFROM entity_weights ewJOIN entity_totals et ON ew.entityid = et.entityidJOIN entity_mf_percent emp ON ew.entityid = emp.entityid),tag_totals AS (SELECT    sector,    SUM(tag_count) AS total_tag_count,    ROUND(SUM(tag_fraction) * 100, 2) AS overall_tag_percentFROM entity_tag_breakdownGROUP BY sector),grand_total AS (SELECT SUM(total_tag_count) AS overall_tag_countFROM tag_totals)SELECTtt.sector,tt.total_tag_count,gt.overall_tag_count,tt.overall_tag_percentFROM tag_totals ttCROSS JOIN grand_total gtORDER BY tt.overall_tag_percent DESC;"    
        data=''
        msgs=executeSql.ExecuteAllNew(sql,data)
        return msgs
     except Exception as e:
